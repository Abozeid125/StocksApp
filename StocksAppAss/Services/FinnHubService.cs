...
namespace StocksAppAss.Services
{
	public class FinnHubService : IFinnHubService
	{
		...
		public async Task<Dictionary<string, object>?> GetCompanyProfile(string stockSymbol)
		{
			using (HttpClient httpClient = _httpClientFactory.CreateClient())
			{
				...
				HttpRequestMessage httpRequestMessage = new HttpRequestMessage
				{
					...
					RequestUri = new Uri($"https://finnhub.io/api/v1/stock/profile2?symbol={stockSymbol}&token={_configuration["StockToken"]}")
		...
		public async Task<List<Dictionary<string, string>>?> GetStocks()
		{
			...
			{
				Method = HttpMethod.Get,
				RequestUri = new Uri($"https://finnhub.io/api/v1/stock/symbol?exchange=US&token={_configuration["StockToken"]}") //URI includes the secret token
			};
